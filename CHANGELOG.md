...
- Adherence to the new `Vibes-No-Log` internal directive.

### Changed
- Improved update process on high-vibes sites to avoid concurrent resources accesses.
...
- [WP-CLI] New command to display Vibes status: see `wp help api status` for details.
- [WP-CLI] New command to toggle on/off main settings: see `wp help api settings` for details.
- Vibes now sends inbound and outbound events to [DecaLog](https://wordpress.org/plugins/decalog/) with settable level.
...
### Added
- It's now possible to use public CDN to serve Vibes scripts and stylesheets (see _Settings | Vibes | Options_).
...
- Vibes database table collation is now `utf8_unicode_ci`.
- The (nag) update message has now a link to display changelog.

### Fixed
- Error while creating Vibes database table with utf8mb4 charset for some version of MySQL.
