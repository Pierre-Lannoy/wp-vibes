...
use Vibes\Plugin\Feature\DecaLog;
...
class Capture {
	...
	private static function clean_endpoint( $host, $endpoint, $bound, $cut = 3 ) {
		 ...
		 * @param   string $bound      Maybe 'inbound', 'outbound' or 'unknown'.
		 */
		$cut = (int) apply_filters( 'vibes_path_level', $cut, $host, $endpoint, $bound );
	...
	private static function record( $response, $args, $url, $bound = 'unknown', $b_in = 0, $b_out = 0 ) {
		...
		try {
			...
			if ( $log_enabled ) {
				\DecaLog\Engine::eventsLogger( VIBES_SLUG )->warning( ucfirst( $bound ) . ' API record: ' . $t->getMessage(), [ 'code' => $t->getCode() ] );
			}
		}
	}
	...
	public static function http_api_debug( $response, $context, $class, $args, $url ) {
		...
		try {
			...
			if ( $log_enabled ) {
				\DecaLog\Engine::eventsLogger( VIBES_SLUG )->warning( 'Outbound API post-analysis: ' . $t->getMessage(), [ 'code' => $t->getCode() ] );
			}
		}
	...
	public static function rest_pre_echo_response( $result, $server, $request ) {
		...
		try {
			...
			\DecaLog\Engine::eventsLogger( VIBES_SLUG )->warning( 'Inbound API pre-analysis: ' . $t->getMessage(), [ 'code' => $t->getCode() ] );
		}
	...
	private static function outbound_stop( $url, $args ) {
		...
		if ( array_key_exists( $id, self::$chrono ) ) {
			...
			\DecaLog\Engine::eventsLogger( VIBES_SLUG )->debug( sprintf( 'Unmatched query for %s.', $url ) );
